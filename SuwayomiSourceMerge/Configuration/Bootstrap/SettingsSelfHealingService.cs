	...
	private static SettingsDocument Merge(SettingsDocument existing, SettingsDocument defaults, ref bool wasHealed)
	{
		...
		return new SettingsDocument
		{
			...
			Scan = new SettingsScanSection
			{
				...
				// Deprecated compatibility setting: preserve only if present; do not regenerate when absent.
				MergeTriggerRequestTimeoutBufferSeconds = existingScan?.MergeTriggerRequestTimeoutBufferSeconds,
			...
			Runtime = new SettingsRuntimeSection
			{
				...
				MetadataApiMinRequestDelayMs = existingRuntime?.MetadataApiMinRequestDelayMs ?? UseDefault(defaultRuntime.MetadataApiMinRequestDelayMs, ref wasHealed),
				MetadataApiMaxRequestDelayMs = existingRuntime?.MetadataApiMaxRequestDelayMs ?? UseDefault(defaultRuntime.MetadataApiMaxRequestDelayMs, ref wasHealed),
